...
| Zone Response Compression Edit     | `compress_response` rules (zone-scoped). Cloudflare's built-in compression covers js/css/json/svg/wasm but omits `model/gltf-binary`, so scene GLBs ship uncompressed — forced brotli is worth ~44% (live-proven 2026-09-11)                                                                                                                                        |
| Zone Transform Rules Edit          | `http_response_headers_transform` and `http_request_transform` rules (zone-scoped). Required to set `Cache-Control` on asset responses, which cannot be done any other way: Workers Static Assets ignores `_headers` files, zone Cache Rules do not apply to Worker-served assets, and `/assets/*` bypasses the Worker. See `docs/workers-static-assets-caching.md` |
| Zone DNS Edit                      | DNS record read/write (zone-scoped) — apex and subdomain cutovers, proxied record management                                                                                                                                                                                                                                                                        |

Skip Queues and AI Gateway unless agents start managing them — 403s name the
missing group, and the extension surfaces them verbatim. The `Zone *` rows are
zone-scoped: under Zone Resources in the builder, include every zone agents
manage rather than only the DNS zone.
