...
| Registrar             | Registrar Domains Admin; Registrar Domains Read                                                                                                                                                            | Domain inventory, expiry, auto-renew, and transfer state. **Effectively read-only** — the API accepts only `auto_renew`/`locked`/`privacy` and exposes no DS/DNSSEC write, so publishing a DS record stays dashboard/registrar work (see `docs/free-plan-hardening.md`) |
